/**
 * VehicleListingEditor — Wix Blocks Dashboard Custom Element
 * Supports: Cars/Trucks, Motorcycles, RVs, Boats
 * Listing Types: For Sale (New/Used), Rent/Lease, Auction
 * Features: Unlimited gallery, video/360 embeds, finance fields, compare, lead capture, related listings
 */
...
class VehicleListingEditor extends HTMLElement {
    ...
    static get observedAttributes() {
        return [
            ...
            'bulk-result', 'makes-list', 'models-list', 'search-results'
        ];
    }
...
vehicle-listing-editor .vle-related-search-bar {
    display: flex;
    gap: 8px;
    margin-bottom: 16px;
}
vehicle-listing-editor .vle-related-search {
    flex: 1;
    padding: 8px 12px;
    border: 1.5px solid var(--border);
    border-radius: 5px;
...
vehicle-listing-editor .vle-related-search:focus { border-color: var(--accent); }
vehicle-listing-editor .vle-related-list {
    background: #fff;
    border: 1px solid var(--border);
    border-radius: var(--r);
    overflow: hidden;
}
vehicle-listing-editor .vle-related-post {
    display: flex;
    align-items: center;
    padding: 12px 16px;
    border-bottom: 1px solid var(--border);
...
vehicle-listing-editor .vle-related-post:last-child { border-bottom: none; }
vehicle-listing-editor .vle-related-post:hover { background: var(--bg2); }
vehicle-listing-editor .vle-related-post.selected { background: #e6f4ff; border-color: var(--blue); }
vehicle-listing-editor .vle-related-check {
    width: 18px;
    height: 18px;
    border: 2px solid var(--border);
    border-radius: 3px;
...
vehicle-listing-editor .vle-related-post.selected .vle-related-check {
    background: var(--blue);
    border-color: var(--blue);
    color: #fff;
}
vehicle-listing-editor .vle-related-info { flex: 1; }
vehicle-listing-editor .vle-related-title {
    font-size: 14px;
    font-weight: 600;
    color: var(--ink);
    margin-bottom: 2px;
}
vehicle-listing-editor .vle-related-slug {
    font-size: 11px;
    color: var(--ink3);
    font-family: 'JetBrains Mono', monospace;
}
vehicle-listing-editor .vle-related-count {
    font-size: 13px;
    color: var(--ink3);
    margin-bottom: 10px;
}
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        ...
        <div class="vle-field" style="margin-top:14px;">
            <label>Additional Safety Features <eml class="vle-tip" data-tip="Type any safety features not listed above, separated by commas.">?</em></label>
...
<div class="vle-section">
    ...
    _relatedPanel() {
        return `
<div class="vle-alert vle-alert-info">${this._icon('link')}<div>Select related listings to show on this listing's detail page. This helps buyers discover similar vehicles and keeps them engaged on your site.</div></div>
<div class="vle-section">
    <div class="vle-section-head">${this._icon('link')} Related Listings</div>
    <div class="vle-section-body">
        <div class="vle-related-search-bar">
            <input type="text" class="vle-related-search" id="relatedSearchInput" placeholder="Search by title, make, model, or year…">
            <button class="vle-btn vle-btn-primary" id="relatedSearchBtn">${this._icon('filter')} Search</button>
        </div>
        <div class="vle-related-count" id="relatedCount">0 listings selected</div>
        <div class="vle-related-list" id="relatedListingsBox"></div>
    </div>
    <div class="vle-section-tip">💡 Recommended: Select 3-6 related listings. Choose vehicles with similar price range, make, or vehicle type for best results.</div>
</div>`;
    }
...
<div class="vle-section">
    ...
    _wire() {
        ...
        const relSearchInp = this.querySelector('#relatedSearchInput');
        const relSearchBtn = this.querySelector('#relatedSearchBtn');
        if (relSearchBtn) {
            relSearchBtn.addEventListener('click', () => {
                const q = relSearchInp.value.trim();
                this._emit('search-related-listings', { query: q, excludeId: this._editItem?._id });
            });
            relSearchInp.addEventListener('keypress', e => {
                if (e.key === 'Enter') {
                    const q = relSearchInp.value.trim();
                    this._emit('search-related-listings', { query: q, excludeId: this._editItem?._id });
                }
...
<div class="vle-card" data-id="${l._id}">
    ...
    _populateEditor(l) {
        ...
        if (Array.isArray(l.relatedListings)) {
            this._data.relatedListings = l.relatedListings;
            this._renderRelatedListingsUI(l.relatedListings);
        }
    }
    ...
    _autoTitle() {
        const d = this._data;
        if (!d.title || this._autoTitleActive) {
    ...
    _onSearchResults(data) {
        ...
        this._renderRelatedListingsUI(results);
    }

    _renderRelatedListingsUI(searchResults) {
        const box = this.querySelector('#relatedListingsBox');
        const cnt = this.querySelector('#relatedCount');
        if (!box) return;

        const selectedIds = this._data.relatedListings || [];
        if (cnt) cnt.textContent = `${selectedIds.length} listing${selectedIds.length !== 1 ? 's' : ''} selected`;

        if (!searchResults || searchResults.length === 0) {
            box.innerHTML = '<div style="padding:20px; text-align:center; color:var(--ink3); font-size:13px;">Search for listings above to add related listings</div>';
            return;
        }

        box.innerHTML = searchResults.map(l => {
            const isSelected = selectedIds.includes(l._id);
            return `
                <div class="vle-related-post ${isSelected ? 'selected' : ''}" data-id="${l._id}">
                    <div class="vle-related-check">${isSelected ? this._icon('check') : ''}</div>
                    <div class="vle-related-info">
                        <div class="vle-related-title">${l.title || `${l.year||''} ${l.make||''} ${l.model||''}`.trim()}</div>
                        <div class="vle-related-slug">${l.slug || ''}</div>
                    </div>
                </div>
            `;
        }).join('');

        box.querySelectorAll('.vle-related-post').forEach(post => {
            post.addEventListener('click', () => {
                ...
                this._data.relatedListings = selectedIds;
                this._renderRelatedListingsUI(searchResults);
            });
        });
    }
