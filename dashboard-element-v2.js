    ...
    connectedCallback() {
        ...
        requestAnimationFrame(() => {
            ...
            if (this._pendingList) {
                try { this._onListingList(JSON.parse(this._pendingList)); this._pendingList = null; } catch (e) { /* ignore */ }
            }
...
vehicle-listing-editor .vle-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(300px,1fr)); gap:16px; padding:20px 22px; }
...
vehicle-listing-editor .vle-compare-table tr:hover td { background:#fafaf8; }
vehicle-listing-editor .vle-compare-table tr:hover td:first-child { background:var(--bg2); }
vehicle-listing-editor .vle-related-search { margin-bottom:14px; }
vehicle-listing-editor .vle-related-search input { width:100%; padding:8px 12px; border:1.5px solid var(--border); border-radius:var(--r); font-family:'Instrument Sans',sans-serif; font-size:13px; background:var(--bg); color:var(--ink); outline:none; }
vehicle-listing-editor .vle-related-search input:focus { border-color:var(--accent); }
vehicle-listing-editor .vle-related-list { background:var(--white); border:1px solid var(--border); border-radius:var(--r2); overflow:hidden; max-height:400px; overflow-y:auto; }
...
vehicle-listing-editor .vle-related-check { width:18px; height:18px; border:2px solid var(--border); border-radius:3px; display:flex; align-items:center; justify-content:center; flex-shrink:0; font-size:11px; font-weight:700; color:transparent; }
...
vehicle-listing-editor .vle-related-thumb-placeholder { width:48px; height:36px; border-radius:4px; flex-shrink:0; background:var(--bg2); display:flex; align-items:center; justify-content:center; color:var(--ink4); }
vehicle-listing-editor .vle-related-thumb-placeholder svg { width:16px; height:16px; }
...
vehicle-listing-editor .vle-related-count { font-size:12px; color:var(--ink3); margin-bottom:10px; font-weight:600; }
...
<div class="vle-host">
    ...
    <div class="vle-editor-view hidden" id="editorView">
        ...
        <div class="vle-tabs">
            <button class="vle-tab active" data-tab="basic">${this._icon('car')} Basic</button>
            ...
            <button class="vle-tab" data-tab="leads">${this._icon('user')} Leads</button>
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-type-grid">
            <button class="vle-type-btn active" data-vtype="car">${this._icon('car')}<span>Cars &amp; Trucks</span></button>
            <button class="vle-type-btn" data-vtype="motorcycle">${this._icon('motorcycle')}<span>Motorcycles</span></button>
            <button class="vle-type-btn" data-vtype="rv">${this._icon('rv')}<span>RVs &amp; Campers</span></button>
            <button class="vle-type-btn" data-vtype="boat">${this._icon('boat')}<span>Boats</span></button>
        </div>
    </div>
</div>
<div class="vle-section">
    ...
    <div class="vle-section-body">
        ...
        <div class="vle-field" style="margin-top:10px;">
            ...
            <select class="vle-sel" data-d="status">
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-grid-2">
            <div class="vle-field vle-full"><label>Listing Title</label><input class="vle-inp" data-d="title" id="titleInp" placeholder="e.g. 2022 Ford Mustang GT Premium"></div>
            <div class="vle-field vle-full"><label>URL Slug</label><input class="vle-inp" data-d="slug" id="slugInp" placeholder="auto-generated" readonly></div>
            <div class="vle-field"><label>Make / Brand</label><input class="vle-inp" data-d="make" list="makesList" placeholder="e.g. Toyota"><datalist id="makesList"></datalist></div>
            <div class="vle-field"><label>Model</label><input class="vle-inp" data-d="model" placeholder="e.g. Camry"></div>
            <div class="vle-field"><label>Trim / Variant</label><input class="vle-inp" data-d="trim" placeholder="e.g. XLE Sport"></div>
            <div class="vle-field"><label>Year</label><input class="vle-inp" data-d="year" type="number" placeholder="2023" min="1900" max="2099"></div>
            <div class="vle-field"><label>Exterior Color</label><input class="vle-inp" data-d="exteriorColor" placeholder="e.g. Midnight Black"></div>
            <div class="vle-field"><label>Interior Color</label><input class="vle-inp" data-d="interiorColor" placeholder="e.g. Beige Leather"></div>
            <div class="vle-field"><label>Body Style</label><input class="vle-inp" data-d="bodyStyle" placeholder="e.g. SUV, Sedan, Pickup"></div>
            <div class="vle-field"><label>Condition</label>
                <select class="vle-sel" data-d="condition">
                    ...
                    <option value="fair">Fair</option><option value="poor">Poor / For Parts</option>
                </select>
            </div>
        </div>
        <div class="vle-field" style="margin-top:14px;"><label>Description</label><textarea class="vle-txt" data-d="description" placeholder="Write a compelling description…" rows="5"></textarea></div>
    </div>
</div>
<div class="vle-section">
    <div class="vle-section-head">${this._icon('star')} Listing Options</div>
    <div class="vle-section-body">
        <div class="vle-tog-row"><div class="vle-tog-info"><div class="vle-tog-label">Featured Listing</div><div class="vle-tog-desc">Highlighted in search results and homepage.</div></div><label class="vle-tog"><input type="checkbox" data-d="featured"><span class="vle-tog-slider"></span></label></div>
        <div class="vle-tog-row"><div class="vle-tog-info"><div class="vle-tog-label">Enable Compare</div><div class="vle-tog-desc">Allow visitors to add this listing to the comparison tool.</div></div><label class="vle-tog"><input type="checkbox" data-d="compareEnabled" checked><span class="vle-tog-slider"></span></label></div>
        <div class="vle-tog-row"><div class="vle-tog-info"><div class="vle-tog-label">Certified Pre-Owned</div><div class="vle-tog-desc">Adds a CPO badge to the listing.</div></div><label class="vle-tog"><input type="checkbox" data-d="certifiedPreOwned"><span class="vle-tog-slider"></span></label></div>
    </div>
</div>`;
    }
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-grid-2">
            <div class="vle-field"><label>VIN</label><input class="vle-inp" data-d="vin" placeholder="e.g. 1HGBH41JXMN109186" maxlength="17"></div>
        ...
        <div class="vle-grid-3">
            ...
            <div class="vle-field"><label>Fuel Type</label>
                <select class="vle-sel" data-d="fuelType">
                    <option value="">— Select —</option><option value="gasoline">Gasoline</option>
                    <option value="diesel">Diesel</option><option value="electric">Electric (EV)</option>
                    <option value="hybrid">Hybrid</option><option value="phev">Plug-in Hybrid (PHEV)</option>
                    <option value="hydrogen">Hydrogen</option><option value="cng">CNG / LPG</option><option value="other">Other</option>
                </select>
            </div>
            <div class="vle-field"><label>Transmission</label>
                <select class="vle-sel" data-d="transmission">
                    <option value="">— Select —</option><option value="automatic">Automatic</option>
                    <option value="manual">Manual</option><option value="cvt">CVT</option>
                    <option value="dct">Dual-Clutch (DCT)</option><option value="semi">Semi-Automatic</option><option value="single">Single-Speed (EV)</option>
                </select>
            </div>
            <div class="vle-field"><label>Drivetrain</label>
                <select class="vle-sel" data-d="drivetrain">
                    <option value="">— Select —</option><option value="fwd">FWD</option>
                    <option value="rwd">RWD</option><option value="awd">AWD</option><option value="4wd">4WD / 4×4</option>
                </select>
            </div>
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-grid-3">
            ...
            <div class="vle-field"><label>Mileage (odometer)</label><input class="vle-inp" data-d="mileage" type="number" placeholder="e.g. 45000"></div>
        </div>
    </div>
</div>
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-grid-4">
            <div class="vle-field"><label>Doors</label><input class="vle-inp" data-d="doors" type="number" placeholder="4"></div>
            <div class="vle-field"><label>Seating</label><input class="vle-inp" data-d="seatingCapacity" type="number" placeholder="5"></div>
            <div class="vle-field"><label>Length (ft)</label><input class="vle-inp" data-d="length" type="number" step="0.1" placeholder="16.5"></div>
            <div class="vle-field"><label>Width (ft)</label><input class="vle-inp" data-d="width" type="number" step="0.1" placeholder="6.5"></div>
            <div class="vle-field"><label>Height (ft)</label><input class="vle-inp" data-d="height" type="number" step="0.1" placeholder="5.2"></div>
            <div class="vle-field"><label>Weight (lbs)</label><input class="vle-inp" data-d="weight" type="number" placeholder="4200"></div>
            <div class="vle-field"><label>Towing (lbs)</label><input class="vle-inp" data-d="towingCapacity" type="number" placeholder="8000"></div>
            <div class="vle-field"><label>Payload (lbs)</label><input class="vle-inp" data-d="payload" type="number" placeholder="1800"></div>
        </div>
    </div>
</div>
<div class="vle-section" id="specBoatSection" style="display:none">
    ...
    <div class="vle-section-body">
        <div class="vle-grid-3">
            ...
            <div class="vle-field"><label>Engine Hours</label><input class="vle-inp" data-d="engineHours" type="number" placeholder="250"></div>
        </div>
    </div>
</div>`;
    }
        ...
        <div class="vle-grid-2">
            <div class="vle-field"><label>Currency</label>
                <select class="vle-sel" data-d="currency">
                    ...
                    <option value="AUD">AUD (A$)</option><option value="INR">INR (₹)</option><option value="AED">AED (د.إ)</option>
                </select>
            </div>
            <div class="vle-field"><label>Asking Price</label><div class="vle-inp-prefix"><span class="vle-inp-prefix-label">$</span><input type="number" data-d="price" placeholder="28500" step="100"></div></div>
            <div class="vle-field"><label>Sale / Discounted Price</label><div class="vle-inp-prefix"><span class="vle-inp-prefix-label">$</span><input type="number" data-d="salePrice" placeholder="26900" step="100"></div></div>
            <div class="vle-field"><label>MSRP / Original Price</label><div class="vle-inp-prefix"><span class="vle-inp-prefix-label">$</span><input type="number" data-d="msrp" placeholder="32000" step="100"></div></div>
        </div>
        <div style="margin-top:14px;">
            <div class="vle-tog-row"><div class="vle-tog-info"><div class="vle-tog-label">Price Negotiable</div><div class="vle-tog-desc">Shows "OBO" badge on listing.</div></div><label class="vle-tog"><input type="checkbox" data-d="priceNegotiable"><span class="vle-tog-slider"></span></label></div>
            <div class="vle-tog-row"><div class="vle-tog-info"><div class="vle-tog-label">Show Price on Site</div><div class="vle-tog-desc">Uncheck to show "Call for Price".</div></div><label class="vle-tog"><input type="checkbox" data-d="showPriceOnSite" checked><span class="vle-tog-slider"></span></label></div>
        </div>
    </div>
</div>
<div class="vle-section" id="rentalPricingSection" style="display:none">
    ...
    <div class="vle-section-body">
        <div class="vle-grid-3">
            <div class="vle-field"><label>Per Day</label><div class="vle-inp-prefix"><span class="vle-inp-prefix-label">$</span><input type="number" data-d="rentalPricePerDay" placeholder="120"></div></div>
            <div class="vle-field"><label>Per Week</label><div class="vle-inp-prefix"><span class="vle-inp-prefix-label">$</span><input type="number" data-d="rentalPricePerWeek" placeholder="700"></div></div>
            <div class="vle-field"><label>Per Month</label><div class="vle-inp-prefix"><span class="vle-inp-prefix-label">$</span><input type="number" data-d="rentalPricePerMonth" placeholder="1800"></div></div>
        </div>
    </div>
</div>
<div class="vle-section" id="auctionPricingSection" style="display:none">
    ...
    <div class="vle-section-body">
        <div class="vle-grid-3">
            <div class="vle-field"><label>Starting Bid</label><div class="vle-inp-prefix"><span class="vle-inp-prefix-label">$</span><input type="number" data-d="auctionStartPrice" placeholder="5000"></div></div>
            <div class="vle-field"><label>Reserve Price</label><div class="vle-inp-prefix"><span class="vle-inp-prefix-label">$</span><input type="number" data-d="auctionReservePrice" placeholder="15000"></div></div>
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-tog-row"><div class="vle-tog-info"><div class="vle-tog-label">Enable Finance Calculator</div><div class="vle-tog-desc">Show a payment estimator on the listing page.</div></div><label class="vle-tog"><input type="checkbox" data-d="financeEnabled"><span class="vle-tog-slider"></span></label></div>
        <div style="margin-top:14px;">
            <div class="vle-grid-3">
                <div class="vle-field"><label>Down Payment ($)</label><div class="vle-inp-prefix"><span class="vle-inp-prefix-label">$</span><input type="number" data-d="downPayment" placeholder="3000" step="500"></div></div>
                <div class="vle-field"><label>Loan Term (months)</label>
                    <select class="vle-sel" data-d="loanTermMonths">
                ...
                <div class="vle-field"><label>Interest Rate (APR %)</label><div class="vle-inp-prefix"><span class="vle-inp-prefix-label">%</span><input type="number" data-d="interestRate" placeholder="5.9" step="0.1" max="99"></div></div>
            </div>
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-grid-2">
            ...
            <div class="vle-field vle-full"><label>External Tour Link</label><input class="vle-inp" data-d="tourUrl" placeholder="https://example.com/virtual-tour"></div>
        </div>
    </div>
</div>`;
    }
    ...
    _checklistHTML(group, items) {
        return items.map(item => `<label class="vle-check-item"><input type="checkbox" data-group="${group}" data-feature="${item}"><span>${item}</span></label>`).join('');
    }
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-checklist">${this._checklistHTML('safety',['Airbags (Front)','Airbags (Side)','Airbags (Curtain)','ABS Brakes','Electronic Stability Control','Traction Control','Lane Departure Warning','Lane Keep Assist','Blind Spot Monitoring','Rear Cross-Traffic Alert','Forward Collision Warning','Automatic Emergency Braking','Adaptive Cruise Control','Parking Sensors (Front)','Parking Sensors (Rear)','Backup Camera','360° Camera','Tire Pressure Monitor','Hill Start Assist','Roll Stability Control'])}</div>
        <div class="vle-field" style="margin-top:14px;"><label>Additional Safety Features</label><input class="vle-inp" data-d="safetyFeatures" placeholder="e.g. Night Vision, Pedestrian Detection"></div>
    </div>
</div>
<div class="vle-section">
    <div class="vle-section-head">${this._icon('gear')} Technology &amp; Connectivity</div>
    <div class="vle-section-body">
        <div class="vle-checklist">${this._checklistHTML('tech',['Apple CarPlay','Android Auto','Wireless CarPlay/Android Auto','Navigation System','Bluetooth','Wi-Fi Hotspot','USB-A Ports','USB-C Ports','Wireless Charging','Premium Audio System','Satellite Radio','Heads-Up Display','Digital Instrument Cluster','Touchscreen Display','Voice Control','Remote Start','Keyless Entry','Push-Button Start','OTA Software Updates','Dashcam'])}</div>
        <div class="vle-field" style="margin-top:14px;"><label>Additional Tech Features</label><input class="vle-inp" data-d="techFeatures" placeholder="e.g. Custom Stereo System"></div>
    </div>
</div>
<div class="vle-section">
    <div class="vle-section-head">${this._icon('car')} Exterior Features</div>
    <div class="vle-section-body">
        <div class="vle-checklist">${this._checklistHTML('exterior',['Alloy Wheels','Chrome Wheels','Running Boards','Roof Rack','Sunroof / Moonroof','Panoramic Sunroof','Convertible Top','Retractable Hard Top','Power Mirrors','Heated Mirrors','Fold-Flat Mirrors','LED Headlights','Adaptive Headlights','Fog Lights','Trailer Hitch','Bed Liner','Tonneau Cover','Spoiler','Body Kit','Tinted Windows'])}</div>
        <div class="vle-field" style="margin-top:14px;"><label>Additional Exterior Features</label><input class="vle-inp" data-d="exteriorFeatures" placeholder="e.g. Custom Wrap, Lift Kit"></div>
    </div>
</div>
<div class="vle-section">
    <div class="vle-section-head">${this._icon('star')} Interior &amp; Comfort</div>
    <div class="vle-section-body">
        <div class="vle-checklist">${this._checklistHTML('interior',['Leather Seats','Heated Front Seats','Heated Rear Seats','Ventilated / Cooled Seats','Memory Seats','Power Driver Seat','Power Passenger Seat','Massage Seats','Third Row Seating','Folding Rear Seats','Heated Steering Wheel','Power Steering','Tilt Steering','Telescoping Steering','Ambient Lighting','Dual-Zone Climate Control','Tri-Zone Climate Control','Rear AC Vents','Power Windows','Power Locks'])}</div>
        <div class="vle-field" style="margin-top:14px;"><label>Additional Interior Features</label><input class="vle-inp" data-d="interiorFeatures" placeholder="e.g. Custom Upholstery"></div>
    </div>
</div>
<div class="vle-section">
    ...
    <div class="vle-section-body">
        ...
        <div class="vle-field" style="margin-top:14px;"><label>All Features (Summary)</label><textarea class="vle-txt" data-d="features" placeholder="Leather Seats, Sunroof, Navigation, Apple CarPlay…" rows="3"></textarea></div>
    </div>
</div>`;
    }
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-grid-2">
            <div class="vle-field"><label>Number of Previous Owners</label>
                <select class="vle-sel" data-d="owners">
                    <option value="">— Select —</option><option value="0">0 (New / Never Titled)</option>
                    <option value="1">1 Previous Owner</option><option value="2">2 Previous Owners</option>
                    <option value="3">3 Previous Owners</option><option value="4+">4 or More Owners</option>
                </select>
            </div>
            <div class="vle-field"><label>Carfax / AutoCheck Report URL</label><input class="vle-inp" data-d="carfaxUrl" placeholder="https://www.carfax.com/VehicleHistory/..."></div>
            <div class="vle-field vle-full"><label>Service History Notes</label><textarea class="vle-txt" data-d="serviceHistory" placeholder="e.g. Full dealer service history available." rows="3"></textarea></div>
        </div>
        <div style="margin-top:14px;">
            <div class="vle-tog-row"><div class="vle-tog-info"><div class="vle-tog-label">Accident History</div><div class="vle-tog-desc">Has this vehicle been in any reported accidents?</div></div><label class="vle-tog"><input type="checkbox" data-d="accidentHistory"><span class="vle-tog-slider"></span></label></div>
        </div>
    </div>
</div>
<div class="vle-section">
        ...
        <div class="vle-grid-3">
            <div class="vle-field"><label>Warranty Type</label>
                <select class="vle-sel" data-d="warrantyType">
                    <option value="">No Warranty (As-Is)</option><option value="factory">Factory / Manufacturer</option>
                    <option value="dealer">Dealer Warranty</option><option value="extended">Extended / Aftermarket</option><option value="cpo">Certified Pre-Owned (CPO)</option>
                </select>
            </div>
            <div class="vle-field"><label>Warranty Remaining (months)</label><input class="vle-inp" type="number" data-d="warrantyMonths" placeholder="36"></div>
            <div class="vle-field"><label>Warranty Miles Remaining</label><input class="vle-inp" type="number" data-d="warrantyMiles" placeholder="30000"></div>
        </div>
    </div>
</div>`;
    }
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-grid-2">
            ...
            <div class="vle-field"><label>Latitude</label><input class="vle-inp" data-d="latitude" type="number" step="0.0001" placeholder="34.0522"></div>
            <div class="vle-field"><label>Longitude</label><input class="vle-inp" data-d="longitude" type="number" step="0.0001" placeholder="-118.2437"></div>
        </div>
    </div>
</div>
<div class="vle-section">
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-tog-row"><div class="vle-tog-info"><div class="vle-tog-label">Enable Lead Capture Form</div><div class="vle-tog-desc">Show a contact form on this listing page.</div></div><label class="vle-tog"><input type="checkbox" data-d="leadCaptureEnabled" checked><span class="vle-tog-slider"></span></label></div>
        <div style="margin-top:16px;" class="vle-grid-2">
            ...
            <div class="vle-field vle-full"><label>Lead Email Recipient</label><input class="vle-inp" data-d="leadEmailRecipient" type="email" placeholder="e.g. leads@citymotors.com"></div>
            <div class="vle-field vle-full"><label>Form Fields to Show</label><input class="vle-inp" data-d="leadFormFields" placeholder="name, email, phone, message, test_drive, financing"></div>
        </div>
    </div>
</div>
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-field"><label>Comparison Highlights</label><textarea class="vle-txt" data-d="compareHighlights" placeholder="One highlight per line:&#10;Only 1 previous owner&#10;Full service history&#10;New tires and brakes" rows="5"></textarea></div>
    </div>
</div>`;
    }
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        ...
        <div class="vle-related-list" id="relatedList">
            <div class="vle-state-box" style="padding:30px;"><p style="font-size:13px;">No other listings available.</p></div>
        </div>
    </div>
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        <div class="vle-grid-2">
            ...
            <div class="vle-field vle-full">
                ...
                <textarea class="vle-txt" data-d="seoDescription" id="seoDescInp" placeholder="e.g. 2022 Ford Mustang GT, 32,000 miles, one owner. Priced at $28,500." rows="3"></textarea>
                <small id="seoDescCount" style="color:var(--ink3);font-size:11px;margin-top:3px;">0 / 160 characters</small>
            </div>
            <div class="vle-field vle-full"><label>SEO Keywords</label><input class="vle-inp" data-d="seoKeywords" placeholder="e.g. 2022 Ford Mustang GT, used Mustang for sale Los Angeles"></div>
        </div>
    ...
    _inject() {
        ...
        while (host.firstChild) this.appendChild(host.firstChild);
    }
    ...
    _wire() {
        ...
        this.querySelector('#sortBy').addEventListener('change', e => {
            ...
            this._sortField = f; this._sortDir = d; this._renderGrid();
        });
        ...
        this.querySelector('#clearCompareBtn').addEventListener('click', () => { this._compareIds = []; this._updateCompareBar(); this._renderGrid(); });
        ...
        this.querySelectorAll('[data-group]').forEach(cb => cb.addEventListener('change', () => this._compileFeatures()));

        const relSearchInp = this.querySelector('#relatedSearchInp');
        let relTimer;
        relSearchInp.addEventListener('input', () => { clearTimeout(relTimer); relTimer = setTimeout(() => this._renderRelatedList(), 250); });
    }
    ...
    async _handleGalleryUpload(files) {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            ph.className = 'vle-gallery-item'; ph.id = `gcard-${blockId}`;
    ...
    _renderRelatedList() {
        ...
        container.innerHTML = available.map(l => {
            ...
            const thumb = l.primaryImage
                ? `<img class="vle-related-thumb" src="${l.primaryImage}" alt="">`
                : `<div class="vle-related-thumb-placeholder">${this._icon('car')}</div>`;
            const meta = [l.year, l.make, l.model, l.price ? '$'+Number(l.price).toLocaleString() : ''].filter(Boolean).join(' · ');
            return `<div class="vle-related-item ${isSelected?'selected':''}" data-rid="${l._id}">
                <div class="vle-related-check">${isSelected?'✓':''}</div>
                ${thumb}
                <div class="vle-related-info">
                    <div class="vle-related-title">${l.title || [l.year,l.make,l.model].filter(Boolean).join(' ') || 'Untitled'}</div>
                    <div class="vle-related-meta">${meta}</div>
                </div>
            </div>`;
        }).join('');
    ...
    _showCompareModal() {
        ...
        const rows = fields.map(([label, key]) => `<tr><td>${label}</td>${items.map(l => `<td>${l[key]||'—'}</td>`).join('')}</tr>`).join('');
        const wrap = this.querySelector('#compareModalWrap');
        wrap.innerHTML = `<div class="vle-modal-overlay"><div class="vle-modal">
            <div class="vle-modal-head"><h2>${this._icon('compare')} Vehicle Comparison</h2><button class="vle-btn vle-btn-light vle-btn-sm" id="closeCompareModal">${this._icon('x')} Close</button></div>
            <div style="padding:20px;overflow-x:auto;"><table class="vle-compare-table"><thead><tr><th>Specification</th>${thCells}</tr></thead><tbody>${rows}</tbody></table></div>
        </div></div>`;
    ...
    _onListingList(data) {
        const loading = this.querySelector('#listLoading');
...
<div class="vle-card" data-id="${l._id}">
    ...
    <div class="vle-card-body">
        <div class="vle-card-title">${l.title || [l.year,l.make,l.model].filter(Boolean).join(' ') || 'Untitled'}</div>
    ...
    _showEditorView() {
        ...
        this.querySelector('#editorTitle').textContent = t ? `Editing: ${t.title || (t.make + ' ' + t.model).trim()}` : 'New Listing';
    ...
    _resetEditorUI() {
        ...
        const cr = this.querySelector('#calcResult');
        if (cr) cr.style.display = 'none';
        this.querySelectorAll('[data-group]').forEach(cb => { cb.checked = false; });
        this._data.relatedListings = [];
        const rc = this.querySelector('#relatedCount');
        if (rc) rc.textContent = '0 listings selected';
    }
    ...
    _populateEditor(l) {
        ...
        this.querySelectorAll('[data-d]').forEach(el => {
            ...
            else el.value = this._data[k] !== null && this._data[k] !== undefined ? this._data[k] : '';
        });
        ...
        const allFeatures = [l.safetyFeatures, l.techFeatures, l.exteriorFeatures, l.interiorFeatures, l.features].filter(Boolean).join(',').toLowerCase();
        this.querySelectorAll('[data-group]').forEach(cb => {
            cb.checked = allFeatures.includes(cb.dataset.feature.toLowerCase());
        });

        const stc = this.querySelector('#seoTitleCount');
        const sdc = this.querySelector('#seoDescCount');
        if (stc && l.seoTitle) stc.textContent = `${l.seoTitle.length} / 60 characters`;
        if (sdc && l.seoDescription) sdc.textContent = `${l.seoDescription.length} / 160 characters`;
    }
    ...
    _toggleVehicleFields() {
        ...
        if (boat) boat.style.display = this._data.vehicleType === 'boat' ? 'block' : 'none';
    }
    ...
    _toggleListingTypeFields() {
        ...
        if (rental) rental.style.display = this._data.listingType === 'rent' ? 'block' : 'none';
        if (auction) auction.style.display = this._data.listingType === 'auction' ? 'block' : 'none';
    }
    ...
    _calcFinance() {
        ...
        const monthly = (loan * (rate/1200)) / (1 - Math.pow(1 + rate/1200, -term));
    ...
    _compileFeatures() {
        ...
        const set = (sel, arr) => { const el = this.querySelector(`[data-d="${sel}"]`); if (el) { el.value = arr.join(', '); this._data[sel] = el.value; } };
        set('safetyFeatures', byGroup.safety);
        set('techFeatures', byGroup.tech);
        set('exteriorFeatures', byGroup.exterior);
        set('interiorFeatures', byGroup.interior);
