...
<div class="vle-card" data-id="${l._id}">
    ...
    _showEditorView() {
        ...
        const publishBtn = this.querySelector('#publishBtn');
        if (t) {
            publishBtn.innerHTML = `${this._icon('check')} Update`;
        } else {
            publishBtn.innerHTML = `${this._icon('check')} Publish`;
        }
    }
    ...
    // ─────────────────────────────────────────────────────────────
    // FIX: _save() now respects the status dropdown value.
    //
    // Previously, clicking "Save Draft" passed status='draft' and
    // clicking "Publish/Update" passed status='active', unconditionally
    // overwriting whatever the user had selected in the Status dropdown.
    //
    // Now:
    //   • "Save Draft" always forces status to 'draft'.
    //   • "Publish/Update" uses whatever is currently in this._data.status
    //     (which is bound directly to the Status dropdown via data-d="status").
    //     If somehow _data.status is empty, it falls back to 'active' only
    //     for brand-new listings that have never been saved.
    // ─────────────────────────────────────────────────────────────
    _save(intent) {
        ...
        let finalStatus;
        if (isDraft) {
            // "Save Draft" button always forces draft regardless of dropdown
            finalStatus = 'draft';
        } else {
            // "Publish / Update" button: use whatever the status dropdown says.
            // _data.status is kept in sync with the dropdown via the data-d listener.
            // Fall back to 'active' only if creating a brand-new listing with no
            // status set (empty freshData), which should not normally happen.
            finalStatus = this._data.status || (this._editItem ? 'active' : 'active');
        }

        // Auto-set soldDate when marking as sold
        if (finalStatus === 'sold' && !this._data.soldDate) {
            const today = new Date().toISOString().split('T')[0];
            this._data.soldDate = today;
            const soldDateEl = this.querySelector('[data-d="soldDate"]');
            if (soldDateEl) soldDateEl.value = today;
        }

        const actionLabel = isDraft
            ? 'Saving draft'
            : (this._editItem ? 'Updating' : 'Publishing');

        this._showProgress(`${actionLabel}…`, 'Preparing listing data…');
