 ...
 *   style-props   — JSON tokens incl. fontSize (px number) and visibleFilters (comma string | 'all')
 *
 * Events dispatched:
 *   page-change          — { page }
...
// ─── SVG icons (stroke-based, themed via currentColor / CSS) ────────────────
const IC = {
...
class VehicleListViewer extends HTMLElement {
  ...
  // ── Build checkbox grids from meta ─────────────────────────────────────────
  _rebuildAdvFilters() {
    ...
    const trigger = () => { this._renderListings(); this._updateAdvBadge(); };
  ...
  _syncRangeSliders() {
    const sr = this.shadowRoot;
    // helper: sets up a dual or single range slider
    const setupDual = (loId, hiId, fillId, loV, hiV, minB, maxB, loLblId, hiLblId, fmtLo, fmtHi) => {
      const lo = sr.getElementById(loId), hi = sr.getElementById(hiId);
      ...
      const update = () => {
        const l = parseInt(lo.value), h = hi ? parseInt(hi.value) : maxB;
        const range = maxB - minB;
        if (fill) {
          fill.style.left  = ((l - minB) / range * 100) + '%';
          fill.style.width = (hi ? (h - l) : l) / range * 100 + '%';
        }
        if (loLbl) loLbl.textContent = fmtLo(l);
        if (hiLbl) hiLbl.textContent = fmtHi(hi ? h : l);
      };
      update();
      lo.addEventListener('input', () => { update(); this._readRanges(); });
      if (hi) hi.addEventListener('input', () => { update(); this._readRanges(); });
    };

    setupDual('vlvPriceMin','vlvPriceMax','vlvPriceFill',
      this._priceRangeMin, this._priceRangeMax, this._priceRangeMin, this._priceRangeMax,
    ...
    setupDual('vlvYearMin','vlvYearMax','vlvYearFill',
      this._yearRangeMin, this._yearRangeMax, this._yearRangeMin, this._yearRangeMax,
      'vlvYearMinLbl','vlvYearMaxLbl',
      v => String(v), v => String(v));

    setupDual('vlvMileageMax', null, 'vlvMileageFill',
      0, this._mileageRangeMax, 0, this._mileageRangeMax,
      null, 'vlvMileageLbl',
      v => v, v => Number(v).toLocaleString() + ' mi');
  }
  ...
  _syncFilterVisibility() {
    ...
    const MAP = {
      vehicleType: 'vlvRowVehicleType', listingType: 'vlvRowListingType', status: 'vlvRowStatus',
      priceRange: 'vlvAdvPriceRange', yearRange: 'vlvAdvYearRange', mileageRange: 'vlvAdvMileageRange',
      make: 'vlvAdvMake', bodyStyle: 'vlvAdvBodyStyle', fuelType: 'vlvAdvFuelType',
      transmission: 'vlvAdvTransmission', drivetrain: 'vlvAdvDrivetrain',
      exteriorColor: 'vlvAdvExteriorColor', featured: 'vlvAdvFeatured',
    };
  ...
  _bindControls() {
    ...
    sr.getElementById('vlvSearch').addEventListener('input', e => {
      ...
      searchTimer = setTimeout(() => { this._searchQuery = e.target.value.toLowerCase().trim(); this._renderListings(); }, 280);
    });

    sr.getElementById('vlvSort').addEventListener('change', e => { this._sortBy = e.target.value; this._renderListings(); });

    const advBtn = sr.getElementById('vlvAdvToggle'), advPanel = sr.getElementById('vlvAdv');
  ...
  _updateAdvBadge() {
    const n = this._filterMakes.length + this._filterBodyStyles.length + this._filterFuelTypes.length +
              this._filterTransmissions.length + this._filterDrivetrains.length + this._filterColors.length +
  ...
  _getFiltered() {
    ...
    if (this._searchQuery) {
      list = list.filter(v => [v.make,v.model,v.trim,v.year,v.vin,v.stockNumber,v.title].join(' ').toLowerCase().includes(this._searchQuery));
    }
  ...
  _cardHTML(v, idx) {
    ...
    const featBadge   = v.featured ? `<span class="vlv-badge vlv-badge-featured"><span class="ic" style="font-size:9px">${IC.star}</span>Featured</span>` : '';
    const imgEl = imgUrl
    ...
    <article class="vlv-card" data-id="${v._id}">
      <div class="vlv-img-wrap">
        ...
        <div class="vlv-cb-wrap"><input type="checkbox" class="vlv-cb" data-id="${v._id}" ${this._compareIds.includes(v._id)?'checked':''}/></div>
      </div>
      <div class="vlv-body">
  ...
  _showCmpModal() {
    ...
    const fields = [
      ...
      ['Drivetrain',v=>v.drivetrain||'—'],['Fuel Type',v=>v.fuelType||'—'],
      ['MPG City',v=>v.mpgCity?v.mpgCity+' mpg':'—'],['MPG Hwy',v=>v.mpgHighway?v.mpgHighway+' mpg':'—'],
  ...
  _renderPagination() {
    ...
    if (start > 1) { html += `<button class="vlv-pbn" data-page="1">1</button>`; if (start > 2) html += `<span class="vlv-pdot">…</span>`; }
    for (let i = start; i <= end; i++) html += `<button class="vlv-pbn ${i===this._currentPage?'active':''}" data-page="${i}">${i}</button>`;
    if (end < this._totalPages) { if (end < this._totalPages-1) html += `<span class="vlv-pdot">…</span>`; html += `<button class="vlv-pbn" data-page="${this._totalPages}">${this._totalPages}</button>`; }
    html += `<button class="vlv-pbn" id="vlvNext" ${this._currentPage===this._totalPages?'disabled':''}>${IC.chevR}</button>`;

    pag.innerHTML = html;
    pag.querySelector('#vlvPrev')?.addEventListener('click', () => { if (this._currentPage > 1) this._changePage(this._currentPage - 1); });
    pag.querySelector('#vlvNext')?.addEventListener('click', () => { if (this._currentPage < this._totalPages) this._changePage(this._currentPage + 1); });
    pag.querySelectorAll('.vlv-pbn[data-page]').forEach(btn => btn.addEventListener('click', () => this._changePage(parseInt(btn.dataset.page))));
  }
