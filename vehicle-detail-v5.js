 ...
 * BUGS FIXED
 * ──────────
 * Fix 7  — All boolean toggle checks updated from strict === false to
 *           falsy check (!v.field). Since the widget now normalizes all
 *           booleans before sending, this is a belt-and-suspenders fix.
 * Fix 8  — Finance calculator guard: if (!v.financeEnabled) return.
 *           Previously: if (!v.financeEnabled && !v.price) — so any
 *           listing with a price showed the calculator regardless of toggle.
 * Fix 9  — Lead form guard: if (!v.leadCaptureEnabled) return.
 *           Previously used strict === false which failed on null.
 * Fix 10 — Related listings: null items filtered before rendering.
 *           Previously a deleted related listing (null in array) would
 *           crash on r.slug access.
 * Fix 5  — Compare checkbox on detail page is only rendered when
 *           compareEnabled is not falsy.
 */
...
const IC = {
    ...
    sold:    `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M8 12l3 3 5-5"/></svg>`,
};
...
function _injectGlobalStyles(s) {
    ...
    const fs = Number(s.fontSize)||14, fs2=Math.max(11,fs-1), fs3=Math.max(10,fs-2);
    el.textContent = `
...
.vdv-status-banner{display:flex;align-items:center;gap:8px;padding:10px 14px;border-radius:8px;font-size:${fs2}px;font-weight:600;background:#374151;color:#9ca3af}
...
.vdv-badge-sold,.vdv-badge-rented,.vdv-badge-expired,.vdv-badge-draft{background:#374151;color:#9ca3af}
...
.vdv-btn-accent{background:${s.btnBg};color:${s.btnText}}.vdv-btn-accent:hover{background:${s.accentHover}}
.vdv-btn-ghost{background:transparent;color:${s.btnGhostText};border:2px solid ${s.btnGhostBorder};text-decoration:none}.vdv-btn-ghost:hover{background:${s.btnGhostBorder};color:${s.btnText}}
...
.vdv-youtube-placeholder{position:relative;cursor:pointer;border-radius:12px;overflow:hidden}
.vdv-youtube-placeholder img{width:100%;height:auto;display:block}
...
.vdv-media-link{display:inline-flex;align-items:center;gap:6px;color:${s.accentColor};font-size:${fs2}px;text-decoration:none;font-weight:600}
...
.vdv-related-card img{width:100%;height:160px;object-fit:cover;display:block}
.vdv-related-img-ph{width:100%;height:160px;background:${s.cardBorder};display:flex;align-items:center;justify-content:center;color:${s.metaColor}}
.vdv-related-info{padding:12px}
.vdv-related-title{font-weight:700;color:${s.titleColor};margin-bottom:4px;font-size:${fs2}px}
.vdv-related-price{font-size:18px;font-weight:800;color:${s.priceColor}}
@media(max-width:1024px){.vdv-container{flex-direction:column;gap:20px}.vdv-sidebar{width:100%;position:static}}
@media(max-width:600px){.vdv-form-row{grid-template-columns:1fr}.vdv-dealer-grid{grid-template-columns:1fr}}
`;
}
    ...
    attributeChangedCallback(name, oldVal, newVal) {
        ...
        if (name === 'vehicle-data') {
            try {
                ...
                // Gallery is already a parsed array (normalized by widget)
                let gallery = [];
                if (Array.isArray(data.gallery)) gallery = data.gallery;
                else if (typeof data.gallery === 'string' && data.gallery) {
                    try { gallery = JSON.parse(data.gallery); } catch(e) { gallery = []; }
                }
                // Ensure primaryImage is first in gallery
  ...
  <div class="vdv-main">
    ...
    <div class="vdv-section" id="vdvCalc" style="display:none">
      ...
      <div class="vdv-calc-grid">
        <div class="vdv-calc-field"><label class="vdv-label" for="calcPrice">Vehicle Price</label><input type="number" class="vdv-input" id="calcPrice"/></div>
        <div class="vdv-calc-field"><label class="vdv-label" for="calcDown">Down Payment</label><input type="number" class="vdv-input" id="calcDown" value="0"/></div>
        <div class="vdv-calc-field"><label class="vdv-label" for="calcRate">Interest Rate (% / yr)</label><input type="number" class="vdv-input" id="calcRate" value="6.9" step="0.1"/></div>
        <div class="vdv-calc-field"><label class="vdv-label" for="calcTerm">Loan Term (months)</label><input type="number" class="vdv-input" id="calcTerm" value="60"/></div>
      </div>
    ...
    <div class="vdv-section" id="vdvLead" style="display:none">
      ...
      <form class="vdv-form" id="vdvLeadForm" onsubmit="return false;">
        <div class="vdv-form-row">
          <div class="vdv-form-field"><label class="vdv-label" for="leadName">Full Name *</label><input type="text" class="vdv-input" id="leadName" required/></div>
          <div class="vdv-form-field"><label class="vdv-label" for="leadEmail">Email *</label><input type="email" class="vdv-input" id="leadEmail" required/></div>
        </div>
        <div class="vdv-form-row">
          <div class="vdv-form-field"><label class="vdv-label" for="leadPhone">Phone</label><input type="tel" class="vdv-input" id="leadPhone"/></div>
          <div class="vdv-form-field"><label class="vdv-label" for="leadContactTime">Preferred Contact Time</label><input type="text" class="vdv-input" id="leadContactTime" placeholder="e.g. Weekday mornings"/></div>
        </div>
        <div class="vdv-form-field"><label class="vdv-label" for="leadMessage">Message</label><textarea class="vdv-input vdv-textarea" id="leadMessage" rows="4" placeholder="Ask about this vehicle…"></textarea></div>
    ...
    _renderSummary(v) {
        ...
        const listBdg = this._badge(v.listingType, v.condition);
        const statusBdg = (v.status && v.status !== 'active')
            ? `<span class="vdv-badge vdv-badge-${v.status}">${v.status}</span>` : '';
        const featBdg = v.featured
            ? `<span class="vdv-badge vdv-badge-featured">${ic('star',11)} Featured</span>` : '';
...
${v.status && v.status !== 'active' ? `<div class="vdv-status-banner">${ic('sold',18)} This vehicle is marked as <strong>${v.status}</strong></div>` : ''}
<div class="vdv-badges">${listBdg}${statusBdg}${featBdg}</div>
...
${v.leadCaptureEnabled ? `<button class="vdv-btn vdv-btn-accent" id="vdvEnquireBtn">${ic('send',16)} Send Enquiry</button>` : ''}
...
${v.priceNegotiable   ? `<div class="vdv-price-note">Negotiable / OBO</div>` : ''}
${!v.showPriceOnSite  ? `<div class="vdv-price-note">Call for Price</div>` : ''}
${v.monthlyPayment    ? `<div class="vdv-price-monthly">Est. ${fmt(v.monthlyPayment)}/mo</div>` : ''}`;
    ...
    _renderTabSpecs(v) {
        const groups = [
            ['Identity',[['VIN',v.vin],['Stock #',v.stockNumber],['Year',v.year],['Make',v.make],['Model',v.model],['Trim',v.trim],['Body Style',v.bodyStyle],['Condition',v.condition],['Ext Color',v.exteriorColor],['Int Color',v.interiorColor]]],
            ['Engine & Performance',[['Engine',v.engine],['Displacement',v.engineDisplacement],['Cylinders',v.cylinders],['Horsepower',v.horsepower?v.horsepower+' hp':null],['Torque',v.torque?v.torque+' lb-ft':null],['Transmission',v.transmission],['Drivetrain',v.drivetrain],['Fuel Type',v.fuelType],['MPG City',v.mpgCity?v.mpgCity+' mpg':null],['MPG Hwy',v.mpgHighway?v.mpgHighway+' mpg':null],['EV Range',v.range?v.range+' mi':null]]],
...
<div class="vdv-specs-group">
  ...
  ${rows.filter(([,val]) => val).map(([l,val]) =>
    `<div class="vdv-spec-row"><span class="vdv-spec-label">${l}</span><span class="vdv-spec-value">${this._esc(String(val))}</span></div>`
  ).join('')}
</div>`).join('')}</div>`;
    }
...
<div>
  <div class="vdv-feature-group-title">${gt}</div>
  <div class="vdv-feature-list">
    ${csv.split(',').map(f => f.trim()).filter(Boolean).map(f =>
      `<div class="vdv-feature-item"><span class="vdv-feature-check">${ic('check',13)}</span>${this._esc(f)}</div>`
    ).join('')}
  </div>
</div>`).join('')}</div>`;
    }
    ...
    _renderTabHistory(v) {
        const cards = [
            ...
            { title:'Accident History',    body: v.accidentHistory ? 'Accident reported — review Carfax for details' : 'No accidents reported' },
            ...
            { title:'Certified Pre-Owned', body: v.certifiedPreOwned ? 'Yes — CPO Certified' : null },
        ].filter(c => c.body);

        this.querySelector('#vdvTabHistory').innerHTML = cards.length
            ? `<div class="vdv-history-grid">${cards.map(c =>
                `<div class="vdv-history-card"><div class="vdv-history-card-title">${c.title}</div><div class="vdv-history-card-body">${c.raw ? c.body : this._esc(c.body)}</div></div>`
              ).join('')}</div>`
  ...
  <img src="https://img.youtube.com/vi/${ytMatch[1]}/hqdefault.jpg" loading="lazy" alt="Video thumbnail">
  <div class="vdv-play-overlay">${ic('play',48)}</div>
</div>`;
            } else {
                html += `<div class="vdv-video-embed"><iframe src="${v.videoUrl}" allowfullscreen loading="lazy" title="Vehicle Video"></iframe></div>`;
            }
        }
        ...
        this.querySelectorAll('.vdv-youtube-placeholder').forEach(placeholder => {
            placeholder.addEventListener('click', () => {
                ...
                placeholder.outerHTML = `<div class="vdv-video-embed"><iframe src="https://www.youtube.com/embed/${id}?autoplay=1" allowfullscreen loading="lazy" title="Vehicle Video"></iframe></div>`;
            });
        });
    }
    ...
    _renderGalleryInMedia() {
        ...
        if (!gallery || !gallery.length || !mainEl) return;
    ...
    _setGalleryImageMedia(idx) {
        ...
        const mainEl = this.querySelector('#vdvGalleryMainMedia');
        if (mainEl) mainEl.innerHTML = `<img class="vdv-gallery-img" src="${url}" alt="${this._esc(alt)}" loading="lazy" decoding="async"/>`;
        this.querySelectorAll('.vdv-thumb').forEach((t, i) => t.classList.toggle('active', i === idx));
    }

    // ─────────────────────────────────────────────────────────────────────
    // FIX 10 — Null guard: related listings are filtered in the widget
    // before sending. But we add a guard here too for safety.
    // ─────────────────────────────────────────────────────────────────────
    _renderRelatedListings(v) {
        // FIX 10: filter nulls (deleted references)
        const related   = (v.relatedListings || []).filter(Boolean);
        ...
        grid.innerHTML = related.map(r => {
            // r can be a full object (from .include()) or an object with at least slug
            if (!r || typeof r !== 'object') return '';
            const title = r.title || `${r.year||''} ${r.make||''} ${r.model||''}`.trim();
            const price = r.salePrice || r.price;
            const imgUrl = this._imgUrl(r.primaryImage, 300, 200, 75);
            return `
<div class="vdv-related-card" data-slug="${r.slug||''}">
  ${imgUrl
    ? `<img src="${imgUrl}" alt="${this._esc(title)}" loading="lazy">`
    : `<div class="vdv-related-img-ph">${ic('car',36)}</div>`}
  <div class="vdv-related-info">
    <div class="vdv-related-title">${this._esc(title)}</div>
    <div class="vdv-related-price">${price ? '$'+Number(price).toLocaleString() : 'Call for Price'}</div>
  </div>
</div>`;
        }).filter(Boolean).join('');
        ...
        grid.querySelectorAll('.vdv-related-card').forEach(card => {
            card.addEventListener('click', () => {
                ...
                if (slug) this.dispatchEvent(new CustomEvent('navigate-to-vehicle', {
    ...
    // ─────────────────────────────────────────────────────────────────────
    // FIX 8 — Finance calculator: only show when financeEnabled is truthy.
    // Old guard: (!v.financeEnabled && !v.price) — showed calc for ANY
    // listing with a price regardless of the toggle.
    // New guard: (!v.financeEnabled) — only shows when explicitly enabled.
    // ─────────────────────────────────────────────────────────────────────
    _renderCalc(v) {
        if (!v.financeEnabled) return;  // FIX 8
    ...
    // ─────────────────────────────────────────────────────────────────────
    // FIX 9 — Lead form: check falsy instead of strict === false.
    // Since widget now normalizes leadCaptureEnabled to a proper boolean,
    // this is belt-and-suspenders. Both !v.leadCaptureEnabled and
    // v.leadCaptureEnabled === false correctly hide the form.
    // ─────────────────────────────────────────────────────────────────────
    _renderLead(v) {
        if (!v.leadCaptureEnabled) return;  // FIX 9
        this.querySelector('#vdvLead').style.display = 'block';
        const titleEl = this.querySelector('#vdvLeadTitle');
        if (titleEl && v.leadFormTitle) {
            titleEl.innerHTML = `${ic('send',20)} ${this._esc(v.leadFormTitle)}`;
        }
    }
    ...
    _imgUrl(raw, w=800, h=600, q=80) {
        ...
        if (raw.startsWith('https://static.wixstatic.com/media/')) {
            try {
                ...
                if (!fn) return raw;
                return `https://static.wixstatic.com/media/${fn}/v1/fill/w_${w},h_${h},al_c,q_${q},enc_avif,quality_auto/${fn}`;
            } catch(e) { return raw; }
        }
