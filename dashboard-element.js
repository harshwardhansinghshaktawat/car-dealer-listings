 ...
 * Features: Unlimited gallery, video/360 embeds, finance fields, compare, lead capture
 */
...
class VehicleListingEditor extends HTMLElement {
    constructor() {
        ...
        this._tab = 'basic';          // active editor tab
        this._listings = [];
        this._editItem = null;
        this._initialized = false;
        this._pendingList = null;
        this._pendingUploads = {};    // blockId → callback (gallery)
        this._pendingMetaProgress = {};
        this._galleryImages = [];     // [{url, alt, isPrimary}]
    ...
    _freshData() {
        return {
            ...
            title: '',
            slug: '',
            listingType: 'used',       // new|used|rent|auction
            vehicleType: 'car',        // car|motorcycle|rv|boat
            status: 'draft',           // draft|active|sold|rented|expired
            featured: false,
            condition: 'used',
            description: '',
            ...
            // Dimensions & Weight (RV/Boat specific too)
            mileage: '',
            length: '',
            width: '',
            height: '',
...
vehicle-listing-editor {
    ...
    --border: #ddd9ce;
    --white: #fff;
    --accent: #e05c1a;    /* deep orange — speed & energy */
    --accent2: #1a3a5c;   /* deep navy — trust & authority */
    --green: #1a7a3a;
    --red: #c0162a;
    --yellow: #d4900a;
    --blue: #1a5ccc;
...
<!-- Boat-specific section (shown/hidden by vehicle type) -->
...
<!-- Rental pricing - shown when listing type = rent -->
...
<!-- Auction pricing - shown when listing type = auction -->
...
<div class="vle-section">
    ...
    <div class="vle-section-body">
        ...
        <div class="vle-tog-row">
            <div class="vle-tog-info">
                <div class="vle-tog-label">Auto-Reply Email</div>
                <div class="vle-tog-desc">Automatically send a thank-you email to the lead after they submit the form.</div>
            </div>
            <label class="vle-tog"><input type="checkbox" data-d="leadAutoReply" checked><span class="vle-tog-slider"></span></label>
        </div>
...
<div class="vle-section">
    ...
    _wire() {
        ...
        // Feature checkboxes → compile features field
    ...
    async _handleGalleryUpload(files) {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            await this._sleep(150); // stagger requests
        }
...
<div class="vle-card" data-id="${l._id}">
    ...
    /* ─────────────────────────────────────────
       COMPILE FEATURES FROM CHECKBOXES
    ───────────────────────────────────────── */
    _compileFeatures() {
        ...
        // Set per-group fields (only if currently empty or was auto-set)
    ...
    /* ─────────────────────────────────────────
       EVENT HANDLERS (from frontend)
    ───────────────────────────────────────── */
    _onSaveResult(data) {
        if (data.success) {
            this._setProgress(100);
            this._setProgressSub('Done!');
            setTimeout(() => { this._hideProgress(); this._toast('success', data.message || 'Listing saved!'); }, 500);
